#!/bin/bash
# Migrate better files from connext_FINAL to root

set -e

echo "=== Migrating Better Files from connext_FINAL ==="
...
cp connext_FINAL/core/orchestrator.py core/orchestrator.py
...
cp connext_FINAL/core/session_manager.py core/session_manager.py
...
cp connext_FINAL/api/routes/admin.py api/routes/admin.py
...
cp connext_FINAL/agents/zero_day_hunter_weaponized.py agents/zero_day_hunter_weaponized.py

# 5. Copy unique files from connext_FINAL
echo "Copying unique files..."
cp connext_FINAL/check_agent.py ./
mkdir -p services
cp connext_FINAL/services/monitoring_service.py services/

# 6. Copy auth_routes.py if different
if [ -f "connext_FINAL/api/routes/auth_routes.py" ]; then
    echo "Copying api/routes/auth_routes.py..."
    cp connext_FINAL/api/routes/auth_routes.py api/routes/
fi
